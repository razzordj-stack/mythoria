...
create table if not exists public.enemy_loot (
  ...
  drop_chance numeric(4,3) not null default 0.25,
  ...
  constraint enemy_loot_chance_valid check(drop_chance>0 and drop_chance<=1),
  constraint enemy_loot_rarity_allowed check(rarity in('common','uncommon','rare','epic','legendary','mythic'))
);
alter table public.enemy_loot enable row level security;
create policy "enemy_loot_read" on public.enemy_loot for select to authenticated using(true);
revoke all on public.enemy_loot from anon;
revoke insert,update,delete on public.enemy_loot from authenticated;
grant select on public.enemy_loot to authenticated;

insert into public.enemy_loot(id,enemy_id,name,description,item_type,rarity,drop_chance,attack_bonus,defense_bonus,magic_bonus,health_bonus,mana_bonus,value) values
('30000000-0000-4000-8000-000000000001','20000000-0000-4000-8000-000000000001','Nebelfell','Silbriges Fell eines Nebelwolfs.','material','common',0.65,0,0,0,0,0,12),
('30000000-0000-4000-8000-000000000002','20000000-0000-4000-8000-000000000002','Goblin-Heiltrank','Ein grob gebrauter, aber wirksamer Heiltrank.','potion','uncommon',0.50,0,0,0,30,0,24),
('30000000-0000-4000-8000-000000000003','20000000-0000-4000-8000-000000000003','Runensplitter','Ein Splitter voller schwacher arkaner Energie.','material','rare',0.40,0,0,2,0,0,45),
('30000000-0000-4000-8000-000000000004','20000000-0000-4000-8000-000000000004','Schattenessenz','Seltene Essenz aus dem Atem eines Schattendrachen.','material','epic',0.30,0,0,4,0,8,95)
on conflict(id) do update set name=excluded.name,description=excluded.description,item_type=excluded.item_type,rarity=excluded.rarity,drop_chance=excluded.drop_chance,attack_bonus=excluded.attack_bonus,defense_bonus=excluded.defense_bonus,magic_bonus=excluded.magic_bonus,health_bonus=excluded.health_bonus,mana_bonus=excluded.mana_bonus,value=excluded.value;

drop function if exists public.perform_combat_action(uuid,text,text);
...
declare
  ...
  v_defending boolean:=false; v_message text; v_status text:='active'; v_cooldown integer:=0; v_effect_penalty integer:=0;
