...
export default function CharactersPage() {
    ...
    const loadCharacters = useCallback(async () => {
        ...
        if (!supabase) {
            setMessage({
                type: "error",
                text:
                    "Die Supabase-Umgebungsvariablen fehlen. Prüfe NEXT_PUBLIC_SUPABASE_URL und deinen Supabase-Schlüssel.",
            });
            setIsLoading(false);
            return;
        }
        ...
        try {
            ...
            if (!user) {
                setMessage({
                    type: "error",
                    text:
                        "Du bist nicht angemeldet. Melde dich an, um deine Charaktere zu sehen.",
                });
                setCharacters([]);
                return;
            }
    ...
    const totalLevels = characters.reduce(
        (sum, character) =>
            sum + Math.max(character.level ?? 1, 1),
        0,
    );

    const totalExperience = characters.reduce(
        (sum, character) =>
            sum + Math.max(character.experience ?? 0, 0),
        0,
    );
    ...
    return (
        <section className="mb-8 grid gap-4 sm:grid-cols-3">
            <StatisticCard
                icon="🧙"
                label="Charaktere"
                value={characterCount.toLocaleString("de-DE")}
            />
...
function CharacterCard({
    ...
    const raceLabel =
        raceLabels[character.race] ??
        formatValue(character.race);

    const classLabel =
        classLabels[character.character_class] ??
        formatValue(character.character_class);

    const portraitKey = character.character_class || character.race;
